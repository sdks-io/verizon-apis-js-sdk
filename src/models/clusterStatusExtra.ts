/**
 * VerizonLib
 *
 * This file was automatically generated by APIMATIC v3.0 ( https://www.apimatic.io ).
 */

import { lazy, object, optional, Schema } from '../schema';
import {
  ServiceLaunchClusterAdditionalParams,
  serviceLaunchClusterAdditionalParamsSchema,
} from './serviceLaunchClusterAdditionalParams';

export interface ClusterStatusExtra {
  vaultIntegration?: ServiceLaunchClusterAdditionalParams;
}

export const clusterStatusExtraSchema: Schema<ClusterStatusExtra> = object({
  vaultIntegration: [
    'vaultIntegration',
    optional(lazy(() => serviceLaunchClusterAdditionalParamsSchema)),
  ],
});
