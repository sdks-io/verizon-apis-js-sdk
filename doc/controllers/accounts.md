...
try {
  const { result, ...httpResponse } = await accountsController.getAccountInformation(aname);
...
try {
  const { result, ...httpResponse } = await accountsController.listAccountStatesAndServices(aname);
  // Get more response info...
  // const { statusCode, headers } = httpResponse;
} catch (error) {
  if (error instanceof ApiError) {
